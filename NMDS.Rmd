...
library(ggpubr)
library(tidyverse)
library(Imap) # for geographic distance
```

```{r}
all_sample_abundance = read.csv("all_sample_abundance.csv", header = T, row.names = 1)
```
#NOT transformed
```{r}
my_data <- all_sample_abundance %>% select(-c(group, species, country, location, site.code,origin,lat,long,date,host,r1,r2,Wolbachia.present)) 
...
```{r}
mantel_test = mantel(dist_geog, dissmatrix)
mantel_test
```
#PERMANOVA
```{r}
#WITH DISTANCE MATRIX
(permanova.specieshost <- adonis(dissmatrix ~ species * host, data = all_sample_abundance, permutations = 999, method = "euclidean"))  # ns
(permanova.specieshost <- adonis(dissmatrix ~ species + host, data = all_sample_abundance, permutations = 999, method = "euclidean"))  # ns
(permanova.country <- adonis(dissmatrix ~ country, data = all_sample_abundance, permutations = 999, method = "euclidean"))  # sig

#WITHOUT DIST MATRIX
(permanova.species <- adonis(dissmatrix ~ species + host + Wolbachia.present, data = all_sample_abundance, permutations = 999, method = "euclidean"))  # ns
